  ...
  return (
    <div className={styles.pagination}>
      ...
      <div className={styles.center}>
        <button type="button" className={styles.pageBtn} disabled={page <= 1 || pages === 0} onClick={() => onChange(page - 1)} aria-label="Предыдущая страница"><UiIcon name="arrowLeft" className={styles.pageIcon} /></button>
        <div className={styles.dots}>
          {Array.from({ length: dotsCount }).map((_, index) => (
            <span key={index} className={`${styles.dot} ${activeDot === index ? styles.dotActive : ""}`} />
          ))}
        </div>
        <button type="button" className={styles.pageBtn} disabled={pages === 0 || page >= pages} onClick={() => onChange(page + 1)} aria-label="Следующая страница"><UiIcon name="arrowLeft" className={`${styles.pageIcon} ${styles.nextIcon}`} /></button>
      </div>
